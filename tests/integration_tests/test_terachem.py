...
from tests.conftest import skipif_no_programs_available


@pytest.mark.integration
@skipif_no_programs_available(["terachem", "terachemd"])
def test_terachem_energy(hydrogen_mol):
    # Modify keywords
    energy_input = SinglePointInput(
        molecule=hydrogen_mol,
        program_args={
            "calc_type": SPCalcType.energy,
            "model": {"method": "hf", "basis": "sto-3g"},
            "keywords": {"purify": "no"},
        },
    )

    for program in ["terachem", "terachemd"]:
        if prog_available(program):
            output = compute(energy_input, program)
            assert output.input_data == energy_input
            assert output.provenance.program == program
            assert np.isclose(output.computed.energy, -0.8884229462, atol=1e-6)
            break  # Don't run again on other program
...
def test_terachem_hessian():
    ...
    for program in ["terachem", "terachemd"]:
        if prog_available(program):
            ...
            # Energy assertion
            assert np.isclose(output.computed.energy, -76.3861099088, atol=1e-6)
