...
def test_qcng_fallback_tried_if_adapter_not_in_qcop(mocker, sp_input):
...
def test_qcng_compute_not_called_if_adapter_in_qcop(mocker, sp_input, sp_test_adapter):
...
def test_qcng_exception_wrapping_raise_exc_true(mocker, sp_input):
...
def test_qcng_exception_wrapping_raise_exc_false(mocker, sp_input):
    # So system check passes for harness and program installation
    mocker.patch("qcop.utils.check_qcng_support")
    qcng_adapter = registry["qcengine"]
